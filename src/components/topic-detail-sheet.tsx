...
export function TopicDetailSheet({ topic, open, onOpenChange }: TopicDetailSheetProps) {
  ...
  const sourceFiles = Array.from(new Set(topic.questions.map(q => q.sourceFile)));
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl lg:max-w-2xl overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            This topic was found in {sourceFiles.length} document{sourceFiles.length === 1 ? '' : 's'} with a total of {topic.questions.length} mention{topic.questions.length === 1 ? '' : 's'}.
          </SheetDescription>
        </SheetHeader>
